...
public class MyScreen extends WindowScreen {
    ...
    public static void checkVersionOnServerJoin() {
        ...
        MeteorExecutor.execute(() -> {
            try {
                String versionString = Http.get("https://picolasclient.com/versions/normal1.21.4.txt").sendString();
                if (versionString != null && !versionString.isEmpty()) {
                    int latestVersion = Integer.parseInt(versionString.trim());


                    if (latestVersion > PicolasClientAddon.MyScreenVERSION) {
        ...
        MeteorExecutor.execute(() -> {
            try {
                ...
                if (versionString != null && !versionString.isEmpty()) {
                    ...
                    latestVersion = PicolasClientAddon.MyScreenVERSION;
                }
    ...
    private void buildUI() {
        addWrappedMessage("Welcome to Picolas Client version checker. Here you can see your current version and check for updates.");

        add(theme.horizontalSeparator()).padVertical(theme.scale(4)).expandX();

        addMessage(String.format("Installed Version: %d", PicolasClientAddon.MyScreenVERSION));
        ...
        if (isVersionFetched && latestVersion > PicolasClientAddon.MyScreenVERSION) {
            addWrappedMessage("You're using an outdated version of the Picolas Client addon. Please update to the latest version. Newer versions may include important bug fixes, improvements, and additional features.");
        } else if (isVersionFetched && latestVersion == PicolasClientAddon.MyScreenVERSION) {
            addWrappedMessage("You're using the latest version of the Picolas Client addon. No update needed.");
        } else {
        ...
        githubButton.action = () -> {
            Util.getOperatingSystem().open("https://github.com/picolas/picolasclient");
        };

        WButton websiteButton = buttonsContainer.add(theme.button("Website")).expandX().widget();
        websiteButton.action = () -> {
            Util.getOperatingSystem().open("https://picolasclient.com");
        };
    }
